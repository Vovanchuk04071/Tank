...
function roadmapSlider() {
  ...
  const setSizes = () => {
    columns = Math.trunc(window.innerWidth / 480);
  ...
  next.addEventListener("click", () => {
    for (let i = 0; i < total; i++) {
      if (states[i].classList.contains("active") && i < total - 1) {
        ...
        states[i + 1].classList.add("active");
        return;
      } else if (states[i].classList.contains("active") && i === total - 1) {
...
function legendWrapper() {
  ...
  const legendImg = document.querySelector(".legend-img-active");
